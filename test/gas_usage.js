...
contract('CharacterCard: Gas Usage', function(accounts) {
	it("deployment: deploying a character cards smart contract requires 5546459 gas", async function() {
		const result = await CharacterCard.new();
		const txHash = result.transactionHash;
		const txReceipt = await web3.eth.getTransactionReceipt(txHash);
		const gasUsed = txReceipt.gasUsed;
		assert.equal(5546459, gasUsed, "character card smart contract deployment gas usage doesn't match: " + gasUsed);
	});

	it("ERC20 transfer: transfer 128 cards requires 4132407 gas", async function() {
		...
		assert.equal(4132407, gasUsed, "transfer gas usage doesn't match: " + gasUsed);
	});

	it("transferCard: transfer a card requires 79699 gas", async function() {
		const card = await CharacterCard.new();
		await card.mint(accounts[0], 0x1);
		const result = await card.transferCard(accounts[1], 0x1);
		const gasUsed = result.receipt.gasUsed;
		assert.equal(79699, gasUsed, "transfer gas usage doesn't match: " + gasUsed);
	});

	it("card updates: set attributes requires 34213 gas", async function() {
		const card = await CharacterCard.new();
		await card.mint(accounts[0], 0x1);
		const result = await card.setAttributes(0x1, 7);
		const gasUsed = result.receipt.gasUsed;
		assert.equal(34213, gasUsed, "set attributes gas usage doesn't match: " + gasUsed);
	});
	...
	it("card updates: remove attributes requires 34497 gas", async function() {
		const card = await CharacterCard.new();
		await card.mint(accounts[0], 0x1);
		await card.setAttributes(0x1, 7);
		const result = await card.removeAttributes(0x1, 2);
		const gasUsed = result.receipt.gasUsed;
		assert.equal(34497, gasUsed, "remove attributes gas usage doesn't match: " + gasUsed);
	});
	...
	it("presale: buying a single card requires 193951 gas", async function() {
		const card = await CharacterCard.new();
		...
		const txReceipt = await web3.eth.getTransactionReceipt(txHash);
		const gasUsed = txReceipt.gasUsed;
		assert.equal(193951, gasUsed, "buying a card gas usage doesn't match: " + gasUsed);
	});
	it("presale: buying three cards requires 450225 gas", async function() {
		const card = await CharacterCard.new();
		const presale = await Presale.new(card.address, accounts[2]);
		await card.addOperator(presale.address, ROLE_CARD_CREATOR);
		...
		const txReceipt = await web3.eth.getTransactionReceipt(txHash);
		const gasUsed = txReceipt.gasUsed;
		assert.equal(450225, gasUsed, "buying three cards gas usage doesn't match: " + gasUsed);
	});
});
