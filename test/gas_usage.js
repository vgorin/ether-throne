...
contract('Gas Usage', function(accounts) {
	it("deployment: deploying a character cards requires 5688228 gas", async function() {
		const card = await CharacterCard.new();
		const txHash = card.transactionHash;
		const txReceipt = await web3.eth.getTransactionReceipt(txHash);
		const gasUsed = txReceipt.gasUsed;
		assert.equal(5688228, gasUsed, "character card deployment gas usage doesn't match: " + gasUsed);
	});
	it("deployment: deploying a presale requires 3065258 gas", async function() {
		const card = await CharacterCard.new();
		const presale = await Presale.new(card.address, accounts[2]);
		const txHash = presale.transactionHash;
		const txReceipt = await web3.eth.getTransactionReceipt(txHash);
		const gasUsed = txReceipt.gasUsed;
		assert.equal(3065258, gasUsed, "presale deployment gas usage doesn't match: " + gasUsed);
	});
	...
	it("transferCard: transfer a card requires 79823 gas", async function() {
		const card = await CharacterCard.new();
		await card.mint(accounts[0], 0x401);
		const result = await card.transferCard(accounts[1], 0x401);
		const gasUsed = result.receipt.gasUsed;
		assert.equal(79823, gasUsed, "transfer gas usage doesn't match: " + gasUsed);
	});

	it("card updates: set attributes requires 34295 gas", async function() {
		const card = await CharacterCard.new();
		await card.mint(accounts[0], 0x401);
		const result = await card.setAttributes(0x401, 7);
		const gasUsed = result.receipt.gasUsed;
		assert.equal(34295, gasUsed, "set attributes gas usage doesn't match: " + gasUsed);
	});
	...
	it("battle: playing a game requires 87149 gas", async function() {
		const card = await CharacterCard.new();
		await card.mint(accounts[0], 0x401);
		await card.mint(accounts[1], 0x402);
		const result = await card.battleComplete(0x401, 0x402, GAME_OUTCOME_DRAW);
		const gasUsed = result.receipt.gasUsed;
		assert.equal(87149, gasUsed, "playing a game gas usage doesn't match: " + gasUsed);
	});
	...
	it("presale: buying three cards requires 487193 gas", async function() {
		const card = await CharacterCard.new();
		const presale = await Presale.new(card.address, accounts[2]);
		await card.addOperator(presale.address, ROLE_CARD_CREATOR);
		...
		const txReceipt = await web3.eth.getTransactionReceipt(txHash);
		const gasUsed = txReceipt.gasUsed;
		assert.equal(487193, gasUsed, "buying three cards gas usage doesn't match: " + gasUsed);
	});
});
