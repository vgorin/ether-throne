...
contract('Presale: Gas Usage', function(accounts) {
	it("presale: buying a single card requires no more then 192050 gas", async function() {
		...
		assert(gasUsed <= 192050, "buying a card gas usage is too high: " + gasUsed);
	});
	it("presale: buying three cards requires no more then 448036 gas", async function() {
		const card = await CharacterCard.new();
		const presale = await Presale.new(card.address, accounts[2]);
		await card.addOperator(presale.address, ROLE_CARD_CREATOR);
		...
		const txReceipt = await web3.eth.getTransactionReceipt(txHash);
		const gasUsed = txReceipt.gasUsed;
		assert(gasUsed <= 448036, "buying three cards gas usage is too high: " + gasUsed);
	});
});
